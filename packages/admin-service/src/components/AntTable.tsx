...
export function AntTable<T extends Record<string, any>>({
  ...
  const handleSelectAll = (checked: boolean) => {
    const keys = checked ? dataSource.map((record, idx) => getRowKey(record, idx)) : [];
  ...
  return (
    <div className={cn('w-full', className)}>
      <div className={cn('relative overflow-auto', scroll?.y && 'max-h-[600px]')}>
        <Table className={cn(sizeStyles[size], bordered && 'border')}>
          <TableHeader className="sticky top-0 bg-muted/50 backdrop-blur supports-[backdrop-filter]:bg-muted/50 z-10">
            <TableRow>
              {rowSelection && (
                      ...
                      <TableCell className={cellPadding[size]}>
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handleSelectRow(record, rowIndex, checked as boolean)
                          }
        ...
        <div className="flex items-center justify-between px-4 py-4 border-t">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              第 {pagination.current} / {Math.ceil((pagination.total || 0) / (pagination.pageSize || 10))} 页
            </span>
            <Button
              variant="outline"
              size="sm"
