...
export interface BaseTemplateFormDialogProps {
  open: boolean;
...
export default function BaseTemplateFormDialog({
  open,
  onOpenChange,
  initialData,
  onSubmit,
  isEdit = false,
}: BaseTemplateFormDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[750px]">
        <DialogHeader>
          <DialogTitle>{isEdit ? '编辑基础模板' : '新增基础模板'}</DialogTitle>
        </DialogHeader>
