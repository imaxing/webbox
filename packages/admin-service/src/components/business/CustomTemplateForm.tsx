...
export interface CustomTemplateFormProps {
...
export interface CustomTemplateFormRef {
  submit: () => Promise<void>;
  cancel: () => void;
}
  ...
  return (
    <div className="space-y-4 max-h-[60vh] overflow-y-auto">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">关联的基础模板标识符</p>
      </div>
        ...
        {baseTemplateVariables.length > 0 && (
          <div className="mt-3">
            {baseTemplateVariables
              .filter(
                (baseVar) => !variablesList.some((v) => v.name === baseVar.name)
              )
              .map((missingVar) => (
                <div
                  key={missingVar.name}
